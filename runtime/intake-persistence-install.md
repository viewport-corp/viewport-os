...
- Viewport OpenClaw `2026.6.11`: do **not** write `channels.telegram.allowBots` or `botLoopProtection`; the installed Telegram adapter accepts peer-bot messages by default and applies its built-in pair-loop guard, while the generated schema rejects those optional keys. Add `${HERMES_BOT_ID}` only to the selected account and exact owner-qualified group/topic pairs supplied as repeated `--boundary=${GROUP_ID}:${TOPIC_ID}` values. Validate every selected account, group, topic, and allowlist before mutating anything. Do not modify unrelated accounts, groups, or topics.
